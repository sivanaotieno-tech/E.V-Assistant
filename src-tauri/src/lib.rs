...
fn spawn_backend(app: &tauri::AppHandle) -> Result<Child, String> {
    ...
    if packaged.exists() {
        ...
        return Command::new(packaged)
            ...
            .stderr(Stdio::null())
            .spawn()
            .map_err(|e| format!("Could not start packaged backend: {e}"));
    }
    ...
    if server.exists() {
        ...
        return Command::new(interpreter)
            ...
            .stderr(Stdio::null())
            .spawn()
            .map_err(|e| format!("Could not start Python backend: {e}"));
    }
                ...
                if let Some(state) = app_handle.try_state::<BackendProcess>() {
                    if let Ok(mut slot) = state.0.lock() {
                        if let Some(mut child) = slot.take() {
                            let _ = child.kill();
                            let _ = child.wait_timeout(Duration::from_millis(300));
                        }
                    }
                }
            }
